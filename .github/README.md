...
- **Any OpenRouter model.** A searchable model menu lists every OpenRouter model that can call tools (about 300), with prices and context sizes. Two are pinned as recommendations: **Nemotron 3 Super** (`nvidia/nemotron-3-super-120b-a12b:free`, $0, the default) and **GPT-6 Astra** (`openai/gpt-6-astra`, the model Unreal Labs benchmarked the harness with). You can also type any model ID. The provider menu lists only providers you have added a key for; OpenAI, a ChatGPT Codex login, Fireworks, and Ollama can be added in Settings.
- **Effort levels** from `low` to `max`, passed to the model as reasoning effort (greyed out for models without reasoning).
